...
const OngoingCallsIn = () => {
	const { inCallsState } = useSockets();
			...
			<div className="w-[100%] h-[400px] overflow-auto  mt-5">
				<table className="w-full ongoing-calls-table">
					...
					<tbody className="text-[11px] ">
						{Array.from(inCallsState.values())
							.reverse()
							.map((row, i) => (
								<tr
								...
								>
									...
									<td>
										{(row.call_status_id == 4)  ? (new Date((Date.now() - row.start_time)).toISOString().slice(14, 19)) : (new Date((row.last_updated - row.start_time)).toISOString().slice(14, 19))}
									</td>
								</tr>
							))}
					</tbody>
