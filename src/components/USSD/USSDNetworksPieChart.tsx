...
const USSDNetworksPieChart = () => {
  const { ussdStats } = useSockets();
  const chartRef = useRef(null);

  let ussdCount: number[] = [];
  let labels: string[] = [];

  ussdStats.network.forEach((count, label) => {
    ...
    <div className="py-5 pb-7 rounded-[5px] border border-card-border shadow-card-shadow">
      <div className="flex justify-between items-center">
        <h3 className="font-semibold px-[19px] text-app-blue leading-[19px]">
          USSD Network Distribution
        </h3>
      </div>

      <div className="w-full min-h-[280px] h-fit  mt-5">
...
export default USSDNetworksPieChart;
